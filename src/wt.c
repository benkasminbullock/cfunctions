...
wrapper_top (char * h_file_name, char ** h_file_guard)
{
    ...
    /* http://www.lemoda.net/c/date-today/index.html

       Get the time and print the time stamp. */

    t = time (0);
    localtime_r (& t, & now);
    strftime (buffer, BSIZE, "at %T on %A %e %B %Y, local time", & now);
